...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form id="editPriorityForm" action="{{ route('priorities.update', $priority->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label for="name">Priority Name</label>
                    <input type="text" name="name" id="name" class="form-control" value="{{ $priority->name }}"
                        required>
